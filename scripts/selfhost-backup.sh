...
# and dumped in one consistent snapshot; the files volume is archived AFTER the
# dump, so every file row in the dump finds its blob (a file deleted in
# between is the only race — stop drobek first for a quiesced backup).
...
(cd "$work" && for f in db.dump files.tar caddy_data.tar; do printf '%s  %s\n' "$(sha256_of "$f")" "$f"; done > SHA256SUMS)
part() { printf '"%s": { "bytes": %s, "sha256": "%s" }' "$1" "$(bytes_of "$work/$1")" "$(sha256_of "$work/$1")"; }
{
  ...
  printf '  "counts": { "apps": %s, "files": %s, "core_migrations": %s },\n' "$apps" "$files_rows" "$core_migrations"
  printf '  "parts": {\n    %s,\n    %s,\n    %s\n  }\n' "$(part db.dump)" "$(part files.tar)" "$(part caddy_data.tar)"
  printf '}\n'
} > "$work/manifest.json"

tmp_archive="$archive.partial"
(umask 077 && tar -C "$work" -czf "$tmp_archive" manifest.json SHA256SUMS db.dump files.tar caddy_data.tar)
chmod 600 "$tmp_archive"
mv "$tmp_archive" "$archive"

say "✓ $archive — $(bytes_of "$archive") bytes in $(( $(date +%s) - started )) s"
say "  apps $apps · files $files_rows · core migrations $core_migrations · image $image (${image_version:-?} ${image_sha:-?})"
