...
|---|---|---|---|
| **D1** | **Prod-beta hostname** | `beta.drobek.app` + `mcp-beta.drobek.app` (single-level), keep apex on its current redirect, add `noindex` | Apex deploy makes the throwaway skeleton the *public launch* on an **HSTS-preloaded** domain (2-yr `includeSubDomains; preload` already shipped) — every 502 during day-1 churn becomes an un-clickthrough-able error. Beta isolates it. **Cost: one fresh HTTP-01 cert** (the kept `drobek.app` cert SAN = apex+www+mcp only; it does NOT cover `beta.*`). Note: `mcp.beta.drobek.app` is 2nd-level and uncoverable by any `*.drobek.app` wildcard — use `mcp-beta.drobek.app`. |
| **D2** | **Object storage** (missing entirely) | Stand up **MinIO** as a shared-service container on the VPS now (or commit to R2/S3 with creds in Phase 0) | The entire product — `deploy_init` presigned PUT → out-of-band upload → content-hash blob store → immutable serving → rollback — is built on S3-compatible storage that **exists nowhere** in the infra (only pg/redis/nginx). Without this, U6 (the actual point of drobek) cannot start and the skeleton can't scaffold its headline test. Belongs in Phase 0. GC/backups follow at M2. |
...
**Goal:** one thread runs end-to-end — local docker dev → both repos build & integrate → GHCR → live on VPS over TLS → drizzle migrate (core **and** private) → `@smoke` green on local AND `beta.drobek.app`. Mirror the proven puls pipeline; apply the five locked decisions.
...
### P0-B · Object storage stood up (D2)
MinIO container as shared service (or R2/S3 creds), bucket `drobek-blobs`, a presigned-PUT round-trip helper. Backups/GC noted as M2 debt.
- **e2e (local+beta):** one real presigned PUT + GET round-trip from a test. This is the ground U6 stands on.
...
- **U5 · MCP OAuth 2.1 Authorization Server** — endpoint + AS (browser consent, discovery metadata, token issue/refresh scoped to workspace+role). *e2e:* add `mcp-beta.drobek.app` in Claude Code → OAuth → consent → `whoami`/tool list; discovery `authorize` not 404. → PHY-71, PHY-53.
...
- **R1 · Object storage missing** (D2) — the product's spine doesn't exist in infra. Blocks U6. *Fix: MinIO/R2 in Phase 0.*
...
- **R5 · Wildcard TLS DNS-01 has no provider named** — estate is 100% HTTP-01 webroot. Gates all M1c. *Fix: spike `*.apps.drobek.app` issuance as a throwaway NOW; identify the drobek.app DNS provider + token.*
...
1. **D1 hostname** — confirm `beta.drobek.app` + `mcp-beta.drobek.app` (accept one fresh HTTP-01 cert; keep apex redirected + `noindex`).
2. **D2 object store** — MinIO-on-VPS vs R2/S3 (decide; it's a new container+disk+GC or external creds+egress).
