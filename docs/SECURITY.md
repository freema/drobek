...
| **Client IP spoofing** | Forged `X-Forwarded-For` / `X-Real-IP` defeats per-IP limits | Behind the bundled Caddy (`TRUST_PROXY=x-real-ip`) only the `X-Real-IP` Caddy sets from the TCP peer is trusted. |
