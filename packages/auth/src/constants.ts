/**
 * The dashboard session cookie's BASE name. On the wire it is
 * `__Host-drobek_session` since M0-06 (apps share the registrable domain) —
 * `cookieName()` in cookies.ts adds the prefix (dropped only on plain-http dev).
 */
